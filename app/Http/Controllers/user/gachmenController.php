...
class gachmenController extends Controller
{
    public function getProduct ($id = null){
    	if($id==null){
    		$product = product::whereRaw('StatusID = ? AND categoryID = ?',['active',5])->get();
    		if($product->count() > 0){
    			return view('user.gachmen',['product' => $product]);
    		}else{
    			
    			return view('user.gachmen')->with(['errors'=> 'Không có dữ liệu']);
    		}
