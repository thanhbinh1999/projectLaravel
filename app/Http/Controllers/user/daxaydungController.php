...
class daxaydungController extends Controller
{
     public function getProduct ($id = null){
    	if($id==null){
    		$product = product::whereRaw('StatusID = ? AND categoryID = ?',['active',3])->get();
    		if($product->count() > 0){
    			return view('user.daxaydung',['product' => $product]);
    		}else{
    			return view('user.daxaydung')->with(['errors'=> 'Không có dữ liệu']);
    		}
